...
export function monthToWeeks(firstDayOfMonth: Date, timezone: Timezone, locale: Locale): Date[][] {
  ...
  while (currentPointer < firstDayNextMonth) {
    ...
    if ((wallTime.day() === locale.weekStart) && week.length > 0) {
      weeks.push(week);
      week = [];
    }
...
export function endingDate(date: Date, timezone: Timezone): Date {
...
export function isOneWholeDay(a: Moment, b: Moment): boolean {
